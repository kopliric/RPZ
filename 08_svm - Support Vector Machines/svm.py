...
def get_kernel(Xi, Xj, options):
    ...
    match options['kernel']:
        ...
        case 'rbf':
            K = np.exp(-(np.linalg.norm(Xi[:, :, np.newaxis] - Xj[:, np.newaxis, :], axis=0) ** 2 /
                         (2 * options['sigma'] ** 2)))
    return K
...
def svm(X, y, C, options):
    ...
    model = {}
    if options is None:
        options = {'verb': True, 'tmax': np.inf}

    K = get_kernel(X, X, options)
    H = (y * y.reshape(-1, 1)) * K
    f = -np.ones(y.shape)
    lb = np.zeros(y.shape[0])
    ub = np.full(y.shape[0], C)

    alphas, fval, t, finished = gsmo(H, f, y, 0, lb, ub, x_init=np.zeros(y.shape[0]),
                                     verb=options['verb'], t_max=options['t_max'])

    sv_idx = np.arange(y.shape[0])[alphas > 10 ** -10]
    model['sv'] = X[:, sv_idx]
    model['y'] = y[sv_idx]
    model['alpha'] = alphas[sv_idx]
    model['options'] = options
    model['b'] = compute_bias(K, y, alphas, C)
    model['fun'] = fval
    return model
...
def classif_svm(X, model):
    ...
    classif = np.sign(np.dot(K.T, model['alpha'] * model['y']) + model['b'])
    return classif
...
def svm_crossvalidation(itrn, itst, X, y, C, options):
    ...
    errors = np.zeros(len(itrn))
    for i in range(len(itrn)):
        model = svm(X[:, itrn[i]], y[itrn[i]], C, options)
        classif = classif_svm(X[:, itst[i]], model)
        errors[i] = np.mean(np.where(classif != y[itst[i]], 1, 0))
...
def compute_measurements_2d(data, normalization=None):
    ...
    height = int(data['images'].shape[0] / 2)
    width = int(data['images'].shape[1] / 2)

    lr = np.sum(imgs[:, :width, :], axis=(0, 1), dtype=np.float64) - np.sum(imgs[:, width:, :],
                                                                            axis=(0, 1), dtype=np.float64)
    ud = np.sum(imgs[:height, :, :], axis=(0, 1), dtype=np.float64) - np.sum(imgs[height:, :, :],
                                                                             axis=(0, 1), dtype=np.float64)
    ...
    lr_norm = (lr - normalization['lr_mean']) / normalization['lr_std']
    ud_norm = (ud - normalization['ud_mean']) / normalization['ud_std']
    X = np.array([lr_norm, ud_norm], dtype=float)
