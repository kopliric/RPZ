...
def parzen(x, x_trn, h):
    ...
    diff_x = x[:, np.newaxis] - x_trn
    K = norm.pdf(diff_x, loc=0, scale=h)
    p = np.sum(K, axis=1)
    p = p / x_trn.shape[0]
    return p
...
def compute_Lh(itrn, itst, x, h):
    ...
    for i in range(10):
        x_tst = x[itst[i]]
        x_trn = x[itrn[i]]
        p = parzen(x_tst, x_trn, h)
        Li[i] = np.sum(np.log(p))
    Lh = float((1/10) * np.sum(Li))
    return Lh
...
def classify_bayes_parzen(x_test, xA, xC, pA, pC, h_bestA, h_bestC):
    ...
    px_A = parzen(x_test, xA, h_bestA)
    px_C = parzen(x_test, xC, h_bestC)

    joint_A = pA * px_A
    joint_C = pC * px_C

    mask = joint_A > joint_C
