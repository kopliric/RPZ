...
def logistic_loss(X, y, w):
    ...
    N = y.shape[0]
    u = y * np.dot(w, X)
    E = 1/N * np.sum(np.log(1 + np.exp(-u)))
    return E
...
def logistic_loss_gradient(X, y, w):
    ...
    N = y.shape[0]
    u = y * np.dot(w, X)
    g = -1/N * np.sum(1/(1 + np.exp(u)) * y * X, axis=1)
    return g
...
def logistic_loss_gradient_descent(X, y, w_init, epsilon):
    ...
    :param epsilon: parameter of termination condition:
                    np.norm(w_new - w_prev) <= epsilon, python float
    ...
    while wt.shape[0] == 1 or np.linalg.norm(wt[-1] - wt[-2]) > epsilon:
        w_new = w - step_size * g
