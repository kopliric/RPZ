...
class Linear(Layer):
    ...
    def backward(self, dL_wrt_output):
        ...
        """
        batch_sz, out_dim = dL_wrt_output.shape

        dL_wrt_x = np.dot(dL_wrt_output, self.W.T)

        self.dL_wrt_W = np.zeros((batch_sz, self.W.shape[0], self.W.shape[1]))
        self.dL_wrt_b = np.zeros((batch_sz, 1, dL_wrt_output.shape[1]))
        for i in range(batch_sz):
            self.dL_wrt_W[i] = np.dot(self.x[i].reshape(-1, 1), dL_wrt_output[i].reshape(1,
                                                                                         out_dim))
            self.dL_wrt_b[i] = dL_wrt_output[i]
